...
import { X, SettingsIcon, BookOpen, Type, Save, Library, Search } from 'lucide-react';
import { Button } from "@/components/ui/button";

const MobileMenu = ({ isOpen, setIsOpen, setShowSettings, setShowSavedCodes, setShowFontPanel, setShowIconPanel, setShowSearchPanel, saveCurrentCode }) => {
  return (
    <div className={`fixed inset-y-0 left-0 w-64 bg-gray-800 shadow-lg z-50 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out`}>
      ...
      <nav className="p-4 flex flex-col space-y-4">
        ...
        <Button onClick={() => { setShowSearchPanel(true); setIsOpen(false); }} className="justify-start">
