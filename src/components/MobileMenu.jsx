...
const MobileMenu = ({ isOpen, setIsOpen, setShowSettings, setShowSavedCodes, setShowFontPanel, setShowIconPanel, setShowCssToolsPanel, setShowHtmlToolsPanel, setShowJsToolsPanel, setShowKeyboardShortcuts, setShowPexelsPanel, setShowConsolePanel, saveCurrentCode }) => {
  return (
    <div className={`fixed inset-y-0 left-0 w-64 bg-gray-900 shadow-lg z-50 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} transition-transform duration-300 ease-in-out overflow-y-auto`}>
      ...
      <nav className="p-4 space-y-2">
        ...
        <MenuButton icon={<Terminal className="w-4 h-4" />} label="Console" onClick={() => { setShowConsolePanel(true); setIsOpen(false); }} />
